      ...
      <section style={{
        ...
        <div style={{ position: 'relative', textAlign: 'center', maxWidth: 680, padding: '0 2rem', animation: 'fadeUp 1s ease both' }}>
          ...
          <p style={{ color: 'rgba(27,42,74,.65)', fontSize: '1rem', letterSpacing: '.04em', marginBottom: '2.5rem', lineHeight: 1.7 }}>
            ...
            {bizConfig?.description || 'Retiro y Delivery'}
          </p>
...
function AuthModal({ onClose, user, showToast }: any) {
  ...
  async function handleRegister() {
    ...
    if (data.user) {
      await sb.from('user_profiles').upsert({ user_id: data.user.id, full_name: form.full_name, email: form.email })
  ...
  const googleBtn = (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '1rem 0' }}>
        <div style={{ flex: 1, height: 1, background: 'rgba(27,42,74,.15)' }} />
        <span style={{ fontSize: '0.72rem', color: 'rgba(27,42,74,.4)', letterSpacing: '.08em' }}>O</span>
        <div style={{ flex: 1, height: 1, background: 'rgba(27,42,74,.15)' }} />
      </div>
      <button onClick={handleGoogle}
        style={{ width: '100%', background: 'white', color: '#1B2A4A', border: '1px solid rgba(27,42,74,.2)', padding: 12, fontFamily: 'inherit', fontSize: '0.85rem', cursor: 'pointer', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontWeight: 500 }}>
        <GoogleIcon />
